...
export const generateReference = (
  ...
  return purchaseType === "plan"
    ? `plan_id=${
        selectedPlan.id
      }-workspace_id=${workspaceId}-workspace_name=${workspaceName}-owner_email=${ownerEmail}-phone_number=${phoneNumber}${assistantsString}${complementsString}${recurringString}-reference${Date.now()}`
    : `assistants_only=true-workspace_id=${workspaceId}-workspace_name=${workspaceName}-owner_email=${ownerEmail}-phone_number=${phoneNumber}${assistantsString}${complementsString}${recurringString}-reference${Date.now()}`;
};
